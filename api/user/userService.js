...
exports.bulkUpdateUsers = async (bulkUpdateOps) => {
    try {
        console.log('Bulk operations array:', bulkUpdateOps);
        await User.bulkWrite(bulkUpdateOps, { ordered: true }, function (err, result) {
            console.log('Bulking!', result);
        });
    } catch (err) {
        console.log(err);
    }
